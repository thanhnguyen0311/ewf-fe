...
const CInventory: FC = () => {
    ...
    const handleCheckboxChange = async (params: any) => {
        const {id, discontinue} = params.data;
        console.log(`Discontinue status for item ID: ${id} is now: ${discontinue}`);
        // try {
        //     // Send an update to the database API
        //     await axios.put(`${process.env.REACT_APP_API_URL}/api/inventory/components/${id}`, {
        //         discontinue: !discontinue, // Toggle value
        //     });
        //
        //     // Update the grid data after a successful API call
        //     params.node.setData({
        //         ...params.data,
        //         discontinue: !discontinue,
        //     });
        //
        //     console.log(`Updated discontinue status for item ID: ${id}`);
        // } catch (error) {
    ...
    const columnDefs: ColDef<ComponentProp>[] = [
        ...
        {
            headerName: "SKU", field: "sku", sortable: true, width: 130, filter: true,
            cellStyle: {fontWeight: "600"}
        },
        {headerName: "CAT", field: "category", width: 90, filter: true, cellStyle: {textAlign: "center"}},
        {
    ...
    const fetchComponents = async () => {
        try {
            ...
            const sortedComponents = response.data
                .filter((component) => {
                    const isInSortOrder = sortOrder.includes(component.sku);
                    return isInSortOrder; // Keep only SKUs present in data_sort
                })
                .sort((a, b) => {
