...
const CInventory: FC = () => {
    ...
    const columnDefs: ColDef<ComponentProp>[] = [
        ...
        },
        {
            ...
            width: 90,
            filter: false,
            cellStyle: {textAlign: "center", fontWeight: "600", backgroundColor: "#7CFFB6"}
        },
        {
            ...
            cellStyle: (params: any) => {
                const styleMap: Record<string, { color: string; backgroundColor: string }> = {
                    "A-Đưa Vào SX": {color: "white", backgroundColor: "#FF00FF"},
                    "L-6 Tháng": {color: "black", backgroundColor: "#FCE5CD"},
                    "K-4 Tháng": {color: "black", backgroundColor: "#F9CB9C"},
                    "I-3 tháng": {color: "black", backgroundColor: "#F6B26B"},
                    "H-2.5 Tháng": {color: "black", backgroundColor: "#EA9999"},
                    "G-2 Tháng": {color: "white", backgroundColor: "#E06666"},
                    "E-45 Ngày": {color: "white", backgroundColor: "#CC0000"},
                    "D-30 Ngày": {color: "white", backgroundColor: "#990000"},
                    "C-15 Ngày": {color: "white", backgroundColor: "#660000"},
                    "B-Het Hàng": {color: "white", backgroundColor: "#FF062A"},
    ...
    const fetchComponents = async () => {
        try {
            ...
            // Sort by the order in data_sort
            const sortedComponents = response.data
                ...
                .filter((component) => {
                    return sortOrder.includes(component.sku); // Keep only SKUs present in data_sort
                })
                .sort((a, b) => {
                    const aIndex = sortOrder.indexOf(a.sku);
                    const bIndex = sortOrder.indexOf(b.sku);
                    return aIndex - bIndex; // Sort by the order in data_sort
                });
        ...
        <>
            ...
            <div className="flex justify-end mb-4">
                <button
                    ...
                    className="bg-brand-300 hover:bg-brand-500 text-white font-semibold py-1.5 px-3 rounded-xl flex items-center shadow-md focus:outline-none focus:ring-2 focus:ring-blue-300"
                >
                    ↻ Refresh
                </button>
            </div>
